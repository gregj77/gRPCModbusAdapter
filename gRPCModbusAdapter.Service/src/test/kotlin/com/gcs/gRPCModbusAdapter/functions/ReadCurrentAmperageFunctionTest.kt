...
internal class ReadCurrentAmperageFunctionTest {
    @Test
    fun `function returns expected name`() {
        val victim = ReadCurrentAmperageFunction(mockk<MessageCRCServiceImpl>(), Schedulers.immediate())

        Assertions.assertThat(victim.functionName).isEqualTo("ReadCurrentAmperage")
    }

    @ParameterizedTest
    @EnumSource(value = RegisterId::class, names = ["CURRENT_AMPERAGE_PHASE1", "CURRENT_AMPERAGE_PHASE2", "CURRENT_AMPERAGE_PHASE3"])
    fun `valid request returns valid response with short value of 0x091b (2331) convertible to float`(registerId: RegisterId) {
        ...
        val victim = ReadCurrentAmperageFunction(crcService, Schedulers.parallel())
        victim.payloadLogger = NOPCommunicationLogger()

        val result = victim.execute(ReadCurrentAmperageFunctionArgs(driverMock, 1, registerId)).block()
        Assertions.assertThat(result).isEqualTo(23.31f)
    ...
    fun `response message which failed CRC check will throw CRC check exception`(registerId: RegisterId) {
        ...
        assertThrows(CrcCheckError::class.java) {
            try {
                victim.execute(ReadCurrentAmperageFunctionArgs(driverMock, 1, registerId)).block()
            } catch (err: Exception) {
                throw err.cause!!
            }
        }
